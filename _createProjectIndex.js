...
fs.openAsync('./idx/'+project.idx, 'w')
.then(function(fd) {
  console.info(fd)
  fs.writeAsync(fd, pb, 0, pb.length)
  .then(function() {
    imgToBuf(project.image)
    .then(function(imgb) {
      console.info(fd)
      fs.appendFileAsync('./idx/'+project.idx, imgb)
      .then(function() {
        ...
        _.each(project.items, function(i, k) {
          ...
          fs.appendFileAsync('./idx/'+project.idx, ji)
          .then(function() {
            ...
            imgToBuf(i.image)
            .then(function(jib) {
              fs.appendFileAsync('./idx/'+project.idx, jib)
              .then(function() {
                insertJobBuffer();
              })
            });
...
.then(function() {
  ...
  var m = new Buffer("MICK RULES!");
  finalBuffer = Buffer.concat([finalBuffer, m], 129673+11);
